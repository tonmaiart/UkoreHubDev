...
class SystemConfigStore:
    ...
    app repo's core/vcs/cloud_sync.py), the same way the MetadataStore
    registry (data/projects.json) is shared. `on_save`, when provided by
    the caller, pushes the freshly-saved file up to the shared bucket.

    This vendored copy never actually reads r2_bucket_name (only
    github_client_id, via updater.py's DEFAULT_GITHUB_CLIENT_ID fallback
    logic) — kept in sync with the app repo's real
    core/storage/config_store.py purely for schema consistency.
    """
