# plugins/studio/program_launcher/
...
`plugins/studio/software_linker/`), not multi-file (see
...
- `plugin.py`:
  ...
  - Resolves the pinned version (`repo.program_version_pins`, same lookup
    ...
    `plugins/studio/software_linker/plugin.py`'s and
    `plugins/studio/maya_launcher/link_resolution.py`'s own copies — keep
  ...
  - No separate per-card buttons — the card itself is the control.
    ...
    only `plugins/studio/maya_launcher`, for anything named "maya") can
...
- `interface/program_launch_registry.py` is a brand-new registry
  ...
  `plugins/studio/maya_launcher/README.md`'s "Standalone launch for
  plugins/studio/program_launcher/" section for the contributing side.
