# plugins/studio/UkoreReferenceEditor/
...
2. **Version-checking** (Maya File tab only) — flags a reference whose file
   ...
   folder) sitting next to it, absorbing `plugins/studio/MayaToolkit`'s old
...
`plugins/studio/MayaToolkit/maya-plug-ins/ukoreMaya.py` /
...
`plugins/studio/MayaToolkit/maya-plug-ins/ukoreMaya.py`; the old standalone
...
`plugins/studio/UkoreBrowser`'s own `MainWindow` directly (cross-plugin
...
`plugins/studio/maya_launcher/plugin.py`'s `open_maya_file` opens with
