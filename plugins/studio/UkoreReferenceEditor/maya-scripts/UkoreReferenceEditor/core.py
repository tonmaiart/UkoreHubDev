...
`plugins/studio/MayaToolkit`'s old "Update All Reference and Picker" menu
...
def open_in_ukore_browser(path: str) -> bool:
    """Opens plugins/studio/UkoreBrowser navigated to `path` (a file or a
...
def auto_check_and_redirect() -> None:
    ...
    for entry in texture_auto:
        redirect_texture(entry.node_name, entry.attr_name, entry.suggested_path)
