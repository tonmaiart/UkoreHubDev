# plugins/core/UkoreReferenceEditor/
...
2. **Version-checking** (Maya File tab only) — flags a reference whose file
   ...
   folder) sitting next to it, absorbing `plugins/core/MayaToolkit`'s old
...
`plugins/core/MayaToolkit/maya-plug-ins/ukoreMaya.py` /
...
`plugins/core/MayaToolkit/maya-plug-ins/ukoreMaya.py`; the old standalone
...
`plugins/core/UkoreBrowser`'s own `MainWindow` directly (cross-plugin
...
`plugins/core/maya_launcher/plugin.py`'s `open_maya_file` opens with
