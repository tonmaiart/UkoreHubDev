...
# ticket has its own Publish Path, its own Playblast/Unreal Export type,
# and its own validation scripts, managed via the "Manage Tickets..."
# button below (PublishApi.ticket_manager_dialog, show_export_type=True
# only for this tool). The old per-repo Repo Studio Setting tab in
# UkoreHub is gone.
# ================================================
...
class MainWindow(ToolkitWindow):
    ...
    def _add_manage_tickets_button(self):
        ...
        identically (RigPublisher/ModelPublisher's own interface.py do the
        exact same thing)."""
    ...
    def open_ticket_manager(self):
        dialog = TicketManagerDialog(
            self, tool_id=function.TOOL_ID, tool_label="AnimationPublisher", show_export_type=True
        )
        dialog.exec_()
        self.initialize_ticket_list_widget()
        self.refresh_publish_destination()
