...
from TonmaiToolkit.core import Misc, Transform, Utility, Connection, Create
...
def create_control(
    ...
    # defalut shape
    pm.circle(ch=False, n=name, nr=dict_normal_vector[Misc.del_neg(axis)])

    # add attribute rotate order
    if display_rotate_order:
        pm.setAttr("{}.rotateOrder".format(name), cb=1)
        pm.setAttr("{}.rotateOrder".format(name), k=1)

    if connect_rotate_order and constraint:
        pm.connectAttr("{}.rotateOrder".format(name), "{}.rotateOrder".format(match))

    # guide ball case
    if guide_ball:
        pm.matchTransform(name, guide_ball, scl=1)
    ...
    pm.makeIdentity(name, a=1, s=1)
    pm.xform(name, ws=1, s=(size, size, size))
    pm.makeIdentity(name, a=1, s=1)
    ...
    if roo:
        ...
        try:
            pm.setAttr(name + ".rotateOrder", pm.getAttr(match + ".rotateOrder"))
        except:
            pass

    # optional : match to given
    if match:
        pm.matchTransform(name, match)
    ...
    if freeze_group:
        ...
        if mirror_freeze_group:
            pm.setAttr("{}.s".format(grp_frz), -1, -1, -1, typ="double3")
    # optional : parent to given
    if parent is not None:
        if freeze_group:
            pm.parent(grp_frz, parent)
        else:
            pm.parent(name, parent)
    ...
    if constraint == "point":
        pm.pointConstraint(name, match, mo=0)
    elif constraint == "orient":
        pm.orientConstraint(name, match, mo=0)
    elif constraint == "parent":
        pm.parentConstraint(name, match, mo=0)
    elif constraint == "scale":
        pm.scaleConstraint(name, match, mo=0)
...
def create_null_shape(object, name):
    # create locator
    node_shape = pm.createNode("nurbsSurface", n=name)
    node_transform = pm.listRelatives(node_shape, p=1, f=1)[0]

    pm.setAttr(node_shape + ".v", 0)

    # parent shape to object
    pm.parent(node_shape, object, s=1, r=1)
    pm.delete(node_transform)

    # generate class_instance to rename
    object_path = (
        pm.listRelatives(object, p=1, f=1)[0]
        if pm.listRelatives(object, p=1, f=1)
        else object
    )
    return_path = "{}|{}".format(object, Misc.cut(node_shape))
...
def create_wrap_deformer(source_wrap, target_wrap):
    ...
    pm.select(source_wrap, target_wrap)
    list_wrap_deformers = pm.ls(type="wrap")
    mc.CreateWrap()
    list_new_wrap_deformers = pm.ls(type="wrap")
    wrap_deformer = list(set(list_wrap_deformers) ^ set(list_new_wrap_deformers))[0]
    wrap_mesh = pm.listConnections(
...
def create_wrap_pin(
    ...
    # create return group
    grp_output = pm.group(em=1, n="{}_output".format(wrap_mesh))
    grp_no_transform = pm.group(em=1, n="{}_wrap_no_transform".format(wrap_mesh))

    # create / get wrap geo
    if custom_wrap_mesh:
        wrap_geo = custom_wrap_mesh
    else:
        wrap_geo = pm.duplicate(wrap_mesh, n=wrap_mesh + "_wrap_output")[0]

    # auto unwrap uv
    if auto_uv:
        shape = pm.listRelatives(wrap_geo, shapes=True)[0]
        pm.polyAutoProjection(shape, ch=True)

    # create wrap deformer
    wrap_deformer, wrap_base = create_wrap_deformer(wrap_geo, wrap_mesh)

    if custom_wrap_mesh:
        pm.parent(wrap_base, grp_no_transform)
    else:
        pm.parent(wrap_geo, wrap_base, grp_no_transform)
    ...
    for i, transform in enumerate(pin_transform):
        ...
        grp_pin_output = pm.group(em=1, p=grp_output)
        ...
        pm.parent(transform, grp_pin_output)
        # pm.pointConstraint(grp_pin_output,transform,mo=1)
...
def create_freeze_group(
    ...
    def get_offset_group_name(object):
        ...
        # check word in scene
        if pm.objExists(grp_freeze_name):
            raise Exception(
    ...
    def match_rotate_order_func():
        ...
        pm.setAttr(grp_freeze + ".rotateOrder", pm.getAttr(obj + ".rotateOrder"))

    def match_object_func():
        if match_object:
            pm.parent(grp_freeze, match_object)
        else:
            pm.parent(grp_freeze, obj)

        # match to target
        Transform.reset_transform(grp_freeze)
        pm.parent(grp_freeze, w=1)
    ...
    for obj in selection:
        grp_freeze = pm.group(em=True, n=get_offset_group_name(object=obj))
        ...
        parent_obj = pm.listRelatives(obj, p=1, f=1)
        (
            pm.parent(grp_freeze, parent_obj[0]) if parent_obj else None
        )  # parent to its parent

        # Parent the object under the freeze group
        pm.parent(obj, grp_freeze)
...
def draw_curve(
    ...
    list_bind_pos = [pm.xform(pos, ws=1, t=1, q=1) for pos in list_items]
    curve = pm.curve(p=list_bind_pos, n=curve_name, d=d)

    if parent:
        pm.parent(curve, parent)

    if rebuild:
        return_curve = pm.rebuildCurve(
            curve,
            ch=0,
            rpo=1,
            rt=0,
...
def draw_nurbs(
    ...
    for curve in [curve1, curve2]:
        if curve == curve1:
            ...
            move = -(size / 2)
        cv_amount = pm.getAttr(curve + ".spans") + pm.getAttr(curve + ".degree")
        pm.select("{}.cv[0:{}]".format(curve, cv_amount - 1))
        pm.move(move, x=1, ls=1, cs=1, wd=1, r=1)
    pm.select(cl=1)

    # loft surface from curve
    ribbon = pm.loft(curve1, curve2, ch=0, rb=0, d=loftDegree, n=name)
    pm.delete(curve1, curve2)

    # parent to new one
    if parent:
        pm.parent(ribbon, parent)

    # rebuild curve
    if rebuild:
        pm.rebuildSurface(ribbon, ch=0, rpo=1, su=su, du=du, sv=sv, dv=dv, kr=0)

    # reverse u and v
    pm.reverseSurface(ribbon, d=3, ch=0, rpo=1)
...
def insert_joint(list_joint, count, typ=0, rename=None):
    ...
    for parent in list_joint:
        ...
        pos_parent = pm.xform(parent, t=1, ws=1, q=1)
        rad_parent = pm.getAttr(parent + ".radius")

        child = pm.listRelatives(parent, typ="joint", c=1)[0]
        pos_child = pm.xform(child, t=1, ws=1, q=1)

        for i in range(count):
            tmp_crv = pm.curve(d=1, p=[pos_parent, pos_child])
            poc = pm.createNode("pointOnCurveInfo")
            pm.connectAttr(tmp_crv + ".worldSpace[0]", poc + ".inputCurve", force=True)

            pm.select(cl=1)
            jnt = pm.joint(n=cut(parent) + "_twist_" + str(i + 1).zfill(2))

            list_return.append(jnt)
            pm.setAttr(jnt + ".radius", rad_parent)

            pm.connectAttr(poc + ".position", jnt + ".translate", force=True)
            param = (1 / (count + 1)) * (i + 1)
            pm.setAttr(poc + ".parameter", param)

            pm.matchTransform(jnt, parent, rot=1, scl=0, pos=0)
            pm.makeIdentity(jnt, r=1, a=1, jo=0)
            pm.delete(tmp_crv)
            ...
            if typ == 0:  # parent to parent
                ...
                if recent_jnt == None:
                    ...
                    pm.parent(jnt, recent_jnt)
                recent_jnt = jnt

        if len(list_joint) == 1:
            pm.parent(child, recent_jnt)

    if rename is not None:
        for i, jnt in enumerate(list_return):
            list_return[i] = pm.rename(jnt, rename[i])
...
def create_uv_pin(transform=None, components=[0]):
    ...
    if Utility.is_py_node(transform):
        transform_node = pm.PyNode(transform)
    else:
        transform_node = transform

    # check if there already is a shape origin node
    shapeOrig = None
    shapes = pm.listRelatives(transform_node, c=True, s=True)
    ...
    if typ == "mesh":
        ...
        pm.error("object provided needs to be Mesh or Nurbssurface")

    # create shape origin if there isn't one
    if shapeOrig == None:
        shape = shapes[0]
        dup = pm.duplicate(shape)
        shapeOrig = pm.listRelatives(dup, c=True, s=True)
        pm.parent(shapeOrig, transform_node, s=True, r=True)
        pm.delete(dup)
        shapeOrig = pm.rename(shapeOrig, "{0}Orig".format(shape))

        # check if inMesh attr has connection
        inConn = pm.listConnections(
            "{0}{1}".format(shape, cAttr), p=True, c=True, d=True
        )

        if inConn:
            pm.connectAttr(inConn[0], "{0}{1}".format(shapeOrig, cAttr))

        pm.connectAttr(
            "{0}{1}".format(shapeOrig, cAttr2), "{0}{1}".format(shape, cAttr), f=True
        )
        pm.setAttr("{0}.intermediateObject".format(shapeOrig), 1)

    # create uvPin Node
    pin = pm.createNode("uvPin", name="{0}_uvPin".format(transform_node))

    pm.connectAttr("{0}{1}".format(shape, cAttr2), "{0}.deformedGeometry".format(pin))
    pm.connectAttr(
...
def draw_curve_circle(list_object, start_cv_index=0, name="curve"):
    circle_transform = pm.circle(s=len(list_object), ch=0, n=name, d=1)[0]
    circle_shape = pm.listRelatives(circle_transform, c=1, s=1)[0]

    cv_amount = pm.getAttr(circle_shape + ".spans")

    # match cv position
    for i in range(cv_amount):
        pos = pm.xform(list_object[i], q=1, t=1, ws=1)
        pm.xform("{}.cv[{}]".format(circle_shape, i), t=pos, ws=1)

    # match last cv to the first cv
    pos = pm.xform(list_object[0], q=1, t=1, ws=1)
    pm.xform("{}.cv[{}]".format(circle_shape, cv_amount), t=pos, ws=1)
...
def create_sticky_gpt(
    ...
    # --- unit conversion ---
    def convert(attr):
        uc = pm.createNode("unitConversion", n=f"{name}_uc")
    ...
    def build_side(attr_zip, attr_dist, side_tag):
        remap = pm.createNode("remapValue", n=f"{name}_{side_tag}_rmv")
        ...
        clamps = []
        for i in range(half):
            md = pm.createNode("multDoubleLinear", n=f"{name}_{side_tag}{i}_mdl")
            adl = pm.createNode("addDoubleLinear", n=f"{name}_{side_tag}{i}_adl")
            pma = pm.createNode("plusMinusAverage", n=f"{name}_{side_tag}{i}_pma")
            cmp = pm.createNode("clamp", n=f"{name}_{side_tag}{i}_cmp")
    ...
    def connect(base, zipped, output, driver, tag):
        cns = pm.parentConstraint(base, zipped, output, mo=1, n=f"{name}_{tag}_pC")[0]
        w = pm.parentConstraint(cns, q=1, wal=1)
        driver.outputR >> cns.attr(w[1])

        rev = pm.createNode("reverse", n=f"{name}_{tag}_rev")
    ...
    if isOdd:
        mid = half - 1
        pma = pm.createNode("plusMinusAverage", n=f"{name}_mid_pma")
        left_clamps[-1].outputR >> pma.input1D[0]
        right_clamps[-1].outputR >> pma.input1D[1]

        cmp = pm.createNode("clamp", n=f"{name}_mid_cmp")
        pma.output1D >> cmp.inputR
        cmp.maxR.set(1)
...
def create_sticky_both(
    ...
    def create_clamp_list_node(attr_zip):
        ...
        node_remap = pm.createNode("remapValue", n="{}_remap_rmv".format(name))
        pm.setAttr(node_remap + ".outputMax", amount_clamp + 1)
        pm.setAttr(node_remap + ".inputMax", 1)
        pm.connectAttr(attr_zip, node_remap + ".inputValue")

        # create sticky systems
        for i in range(amount_clamp):
            node_md_distance = pm.createNode(
                "multDoubleLinear", n="{}_zipper_mdl".format(name)
            )
            pm.connectAttr(attr_distance, node_md_distance + ".input2")
            pm.setAttr(node_md_distance + ".input1", i + offset)

            node_adl_add = pm.createNode(
            ...
            node_pma_output = pm.createNode(
                "plusMinusAverage", n="{}_zipper_pma".format(name)
            )
            pm.setAttr(node_pma_output + ".operation", 2)
            pm.connectAttr(node_remap + ".outValue", node_pma_output + ".input1D[0]")
            pm.connectAttr(node_adl_add + ".output", node_pma_output + ".input1D[1]")

            node_clamp = pm.createNode("clamp", n="{}_zipper_cmp".format(name))
            pm.connectAttr(node_pma_output + ".output1D", node_clamp + ".inputR")
    ...
    def apply_sticky():
        def connect_constraint(index, node_clamp):
            ...
            list_weight = pm.parentConstraint(constraint, q=1, wal=1)

            pm.connectAttr(node_clamp + ".outputR", constraint + "." + list_weight[1])

            node_reverse = pm.createNode("reverse", n="{}_zipper_rev".format(name))
            pm.connectAttr(node_clamp + ".outputR", node_reverse + ".inputX")
            pm.connectAttr(node_reverse + ".outputX", constraint + "." + list_weight[0])
        ...
        if isOdd:
            ...
            node_clamp_sum = pm.createNode(
                "clamp", n="{}_betweenClamp_cmp".format(name)
            )
            pm.connectAttr(node_pma_sum + ".output1D", node_clamp_sum + ".inputR")

            pm.setAttr(node_clamp_sum + ".minR", 0)
            pm.setAttr(node_clamp_sum + ".maxR", 1)
    ...
    for attr in list_attr_zip:
        node_uc = pm.createNode("unitConversion", n="{}_zipper_ud".format(name))
        pm.setAttr(node_uc + ".conversionFactor", 1 / unit_conversion)
        pm.connectAttr(attr, "{}.input".format(node_uc))
...
def create_sticky_matrix(
    ...
    for i in range(amount):
        ...
        if isMirror and i >= (index_mirror - 1):
            ...
            node_set_range = pm.createNode("setRange", n="sr_{}{}".format(name, i + 1))
            node_md = pm.createNode("multiplyDivide", n="md_{}{}".format(name, i + 1))
            node_pma = pm.createNode(
            ...
            pm.connectAttr(attr_zipper, "{}.valueX".format(node_set_range))
            pm.setAttr("{}.minX".format(node_set_range), 0)
            pm.setAttr("{}.maxX".format(node_set_range), 1)
            pm.connectAttr(
            ...
            # multiply + pma node
            pm.connectAttr(attr_size, "{}.input1X".format(node_md))
            pm.connectAttr(attr_size, "{}.input1Y".format(node_md))

            pm.setAttr("{}.input2X".format(node_md), value_mult)
            pm.setAttr("{}.input2Y".format(node_md), value_mult)
            ...
            pm.setAttr("{}.input3D[1].input3Dx".format(node_pma), input_min)
            pm.setAttr("{}.input3D[1].input3Dy".format(node_pma), input_max)
...
def create_sticky_old(
    ...
    def attr_zip_converision_unit():
        ...
        pm.setAttr(node_uc + ".conversionFactor", 1 / unit_conversion)
        pm.connectAttr(attr_zip, "{}.input".format(node_uc))

        return "{}.output".format(node_uc)

    def create_sticky_systems():
        # prepare remap and transform node
        node_remap = pm.createNode("remapValue")
        pm.setAttr(node_remap + ".outputMax", amount_clamp + 1)
        pm.setAttr(node_remap + ".inputMax", 1)
        pm.connectAttr(attr_zip, node_remap + ".inputValue")

        node_adjust = pm.createNode("transform", n="{}_configSticky".format(name))
        pm.parent(node_adjust, config_parent)

        # create sticky systems
        for i in range(amount_clamp):
            node_md_distance = pm.createNode("multDoubleLinear")
            pm.connectAttr(attr_distance, node_md_distance + ".input2")
            pm.setAttr(node_md_distance + ".input1", i + offset)

            node_adl_add = pm.createNode("addDoubleLinear")
            pm.connectAttr(
                "{}.output".format(node_md_distance), node_adl_add + ".input2"
            )
            pm.addAttr(
            ...
            node_pma_output = pm.createNode("plusMinusAverage")
            pm.setAttr(node_pma_output + ".operation", 2)
            pm.connectAttr(node_remap + ".outValue", node_pma_output + ".input1D[0]")
            pm.connectAttr(node_adl_add + ".output", node_pma_output + ".input1D[1]")
...
def create_zipper_both(
    ...
    for i in range(count):
        node_add = pm.createNode("addDoubleLinear")
        node_clamp = pm.createNode("clamp")
...
def create_zipper_simple(
    ...
    node_metadata = pm.createNode("transform", n="zipper_data")

    # create matrix attribute
    pm.addAttr(node_metadata, ln="upperInMatrix", at="matrix", multi=True)
    pm.addAttr(node_metadata, ln="lowerInMatrix", at="matrix", multi=True)
    pm.addAttr(node_metadata, ln="betweenMatrix", at="matrix", multi=True)
    pm.addAttr(node_metadata, ln="upperOffsetMatrix", at="matrix", multi=True)
    pm.addAttr(node_metadata, ln="lowerOffsetMatrix", at="matrix", multi=True)
    pm.addAttr(node_metadata, ln="upperOutputMatrix", at="matrix", multi=True)
    pm.addAttr(node_metadata, ln="lowerOutputMatrix", at="matrix", multi=True)

    # create zip attribute
    pm.addAttr(node_metadata, ln="zipEach", at="float", min=0, max=1, multi=True)
    pm.addAttr(node_metadata, ln="zipValue", at="float", min=0, max=1)
    pm.addAttr(node_metadata, ln="zipRemap", at="float", min=0, max=1, multi=True)

    # add attribute to controller
    if controller:
        pm.addAttr(controller, ln="zip", k=True, min=0, max=10)
    ...
    for upper_driver, lower_driver, upper_target, lower_target in zip(
        ...
        if matrix_connection:
            ...
            node_blendMatrix = pm.createNode("blendMatrix")
            pm.connectAttr(
                "{}.upperInMatrix[{}]".format(node_metadata, i),
            ...
            pm.setAttr("{}.target[0].weight".format(node_blendMatrix), 0.5)
            pm.connectAttr(
            ...
            loc_upper = pm.spaceLocator(n="{}_upper_loc".format(upper_target))
            ...
            loc_lower = pm.spaceLocator(n="{}_lower_loc".format(lower_target))
...
def create_stretchy_joint(
    ...
    def create_pole_lock():
        ...
        # node_bc_lock = pm.createNode("blendColors", n="{}_stretchBlend_bc".format(name_tag))
        # pm.connectAttr("{}.output3Dx".format(node_pma_stretch), "{}.color2R".format(node_bc_lock))
        # pm.connectAttr("{}.output3Dy".format(node_pma_stretch), "{}.color2G".format(node_bc_lock))
        # pm.connectAttr(attr_elbow_lock, "{}.blender".format(node_bc_lock))

        for i in range(len_stretch_joint):
            if i + 1 >= len_stretch_joint:
                continue
            else:
                node_dist = pm.createNode(
                    "distanceBetween",
        ...
        # Pole Lock ----------------------------------
        # node_md_lock = pm.createNode("multiplyDivide", n="{}_lockConversion_mdv".format(name_tag))
        # pm.setAttr("{}.input2X".format(node_md_lock), invert_value)
        # pm.setAttr("{}.input2Y".format(node_md_lock), invert_value)
        # pm.connectAttr("{}.distance".format(node_dist_up), "{}.input1X".format(node_md_lock))
        # pm.connectAttr("{}.distance".format(node_dist_low), "{}.input1Y".format(node_md_lock))
        # pm.connectAttr("{}.outputX".format(node_md_lock), "{}.color1R".format(node_bc_lock))
        # pm.connectAttr("{}.outputY".format(node_md_lock), "{}.color1G".format(node_bc_lock))
    ...
    def connect_stretch():
        ...
        for i in range(len_stretch_joint):
            ...
            pm.setAttr(
                ...
                pm.getAttr("{}.t{}".format(stretch_joint, axis_forward_abs)),
            )  # translate multiply

            # by pass
            node_bc_bypass = pm.createNode(
                "blendTwoAttr",
            ...
            pm.setAttr(
                ...
                pm.getAttr("{}.output".format(node_md_stretch)),
            )
    ...
    for i in range(len_stretch_joint - 1):
        ...
        pm.addAttr(controller_attr, k=1, ln="LimbStretch{}".format(i + 1), at="float")
    ...
    pm.addAttr(controller_attr, k=1, ln="elbowLock", at="float", min=0, max=1)
    pm.addAttr(controller_attr, k=1, ln="autoStretch", at="float", min=0, max=1)
...
def create_switch_fk_ik_chain(
    ...
    def duplicate_joints():
        ...
        for i, typ in enumerate(list_append_keyword):
            ...
            # parent offset joint
            parent_joint = pm.listRelatives(target_joints[0], p=1, f=1)

            if parent_joint:
                tmp = pm.duplicate(parent_joint[0], n=parent_joint[0] + typ, po=1)[0]

                # unlock translate,rotate,scale
                Transform.lock_attributes(tmp, t=1, r=1, s=1, v=1, l=0, k=1)

                pm.parent(tmp, parent_path)
                pm.parentConstraint(parent_joint[0], tmp)
                recent = tmp

            # limb joint
            for joint in target_joints:
                tmp = pm.duplicate(joint, n=joint + typ, po=1)[0]
                (
                    pm.parent(tmp, parent_path)
                    if recent is None
                    else pm.parent(tmp, recent)
                )
                recent = tmp

    def connect_switch_joints():
        # create switch systems , use blendColor Node
        node_reverse = pm.createNode("reverse", n="rev_{}Switch".format(tag_name))
        pm.connectAttr(attr_switch, "{}.inputX".format(node_reverse))
        ...
        for i, joint in enumerate(target_joints):
            for data in [["t", "Pos"], ["r", "Rot"]]:
                ...
                node_blend = pm.createNode(
                    "blendColors", n="{}_blend{}{}_blc".format(tag_name, joint, keyword)
                )
                pm.connectAttr(attr_switch, "{}.blender".format(node_blend))
                pm.connectAttr(
    ...
    def connect_switch_visibility():
        # connect visibility
        node_rev = pm.createNode("reverse", n="{}_switchVis_rev".format(tag_name))
        pm.connectAttr(attr_switch, "{}.inputX".format(node_rev))
        pm.connectAttr("{}.inputX".format(node_rev), "{}.v".format(grp_ik_controls))
        pm.connectAttr("{}.outputX".format(node_rev), "{}.v".format(grp_fk_controls))
...
def create_twist_chain(
    ...
    for i in range(len(list_twist)):
        ...
        node_md = pm.createNode("multiplyDivide", n="md_{}".format(twist_transform))

        invert_value = 1 if invert is False else -1
        pm.setAttr("{}.operation".format(node_md), 1)
...
def create_finger_rig(
    ...
    def create_finger_ik():
        def create_thumb():
            ...
            pm.parent(ik_handle, list_finger_control[2])
            ...
            pm.poleVectorConstraint(list_finger_control[1], ik_handle)

            pm.pointConstraint(list_finger_control[0], list_chain_ik[0])

            # add blend space option
            pm.addAttr(list_finger_control[2], ln="world", k=1, min=0, max=1)
            ...
            pm.pointConstraint(
                ...
                pm.listRelatives(list_finger_control[1], p=1)[0],
                mo=1,
            )

        def create_finger():
            pm.addAttr(list_finger_control[3], ln="lockFingerTip", k=1, min=0, max=1)
            ...
            pm.pointConstraint(list_finger_control[0], list_lock_chain[0])
            ...
            pm.pointConstraint(list_finger_control[0], list_no_lock_chain[0])
            ...
            # parent chain -------------------------------
            pm.parent(list_finger_control[2], list_finger_control[3])
            ...
            pm.orientConstraint(list_finger_control[2], list_lock_chain[2])

            # add blend space option ------------------------
            pm.addAttr(list_finger_control[3], ln="world", k=1, min=0, max=1)
            ...
            pm.connectAttr(attr_lock, "{}.v".format(list_finger_control[2]))
    ...
    def create_fingers_fk():
        ...
        for i in range(len(list_chain_fk)):
            ...
            grp_offset = pm.listRelatives(control_name, p=1)[0]

            # chain in hierarchy
            if recent_control:
                pm.parent(grp_offset, recent_control)
            else:
                pm.parent(grp_offset, grp_fk_control)
    ...
    def duplicate_joints(list_append_keyword=["Fk", "Ik"]):
        ...
        for i, typ in enumerate(list_append_keyword):
            ...
            # parent offset joint
            parent_joint = pm.listRelatives(list_finger_joint[0], p=1, f=1)
            ...
            for joint in list_finger_joint:
                tmp = pm.duplicate(joint, n="{}_{}".format(cut(joint), typ), po=1)[0]
                (
                    pm.parent(tmp, parent_path)
                    if recent is None
                    else pm.parent(tmp, recent)
    ...
    def create_switch_systems(
        ...
        # create switch systems , use blendColor Node
        node_reverse = pm.createNode("reverse", n=cname(tag_name, "switch", "rev"))
        pm.connectAttr(attr_switch, "{}.inputX".format(node_reverse))
    ...
    def create_finger_main_control():
        ...
        pm.parentConstraint(list_finger_joint[0], ctrl_main)
        pm.addAttr(ctrl_main, ln="FkIk", k=1, min=0, max=1)

    grp_finger_rig = pm.group(em=1, n="{}_rig_{}".format(tag_name, grp))

    grp_fk_control = pm.group(
        em=1, p=grp_finger_rig, n="{}_fk_control_{}".format(tag_name, grp)
    )
    grp_ik_control = pm.group(
    ...
    if parent:
        pm.parent(grp_finger_rig, parent)
...
def absolute_poly_bridge_edge():
    ...
    for i in range(len(part_A)):
        pm.polyBridgeEdge(part_A[i], part_B[i], divisions=0)


def create_line_annotate(list_object, name="line", parent=None):
    list_attr_position = []

    for obj in list_object:
        node_dcm = pm.createNode("decomposeMatrix")
        obj.worldMatrix[0] >> node_dcm.inputMatrix
        list_attr_position.append(node_dcm.outputTranslate)

    # create curve
    list_point = [(0, 0, 0) for i in range(len(list_attr_position))]
    curve = pm.curve(p=list_point, d=1, n="{}_crv".format(name))
    curve_shape = pm.listRelatives(curve, c=1, s=1)[0]

    # connect position
    for i, attr_pos in enumerate(list_attr_position):
        attr_pos >> curve_shape.controlPoints[i]

    # parent curve
    curve.inheritsTransform.set(False)
    pm.parent(curve_shape, parent)

    # set override
    Utility.lock_attribute(curve, t=True, r=True, s=True, v=True)
    curve.overrideEnabled.set(True)
    curve.overrideDisplayType.set(2)
...
def create_blend_joint_world_space(jointA, jointB):
    def create_fake_orient_rotation():
        node_mm_parent = pm.createNode("multMatrix", n="parent_mm")
        node_mm_parent = pm.createNode("multMatrix", n="parent_mm")
        node_mm_parent = pm.createNode("multMatrix", n="parent_mm")

    output_joint = pm.createNode("joint", n="joint_output")
...
def create_world_vector(objectA, objectB):
    ...
    locatorA = pm.spaceLocator(n="{}_vecPointA".format(objectA))
    locatorB = pm.spaceLocator(n="{}_vecPointB".format(objectB))

    Utility.match_parent(locatorA, objectA)
    Utility.match_parent(locatorB, locatorA)

    locatorA_shape = pm.listRelatives(locatorA, c=1, s=1)[0]
    locatorB_shape = pm.listRelatives(locatorB, c=1, s=1)[0]

    pm.xform(locatorB, os=True, t=(1, 0, 0))

    constraint = pm.aimConstraint(objectB, locatorA, aim=[1, 0, 0])
...
def create_skirt_auto_rot(
    ...
    if not skirtB:
        skirtB = pm.spaceLocator(n="tmp")
        pm.matchTransform(skirtB, skirtA)
        pm.xform(skirtB, ws=True, r=True, t=(0, -1, 0))
    ...
    locator_B_child = pm.listRelatives(locatorB, c=1, typ="transform")[0]
    pm.parent(locator_B_child, w=1)

    Transform.reset_transform(locatorB)
    pm.parent(locator_B_child, locatorB)
    ...
    # Get Angle Between
    node_angle = pm.createNode("angleBetween")
...
def create_blend_joint(joint, joint_parent=None):
    joint_blend = pm.createNode("joint", n="{}_blend".format(joint))

    # create node
    node_pair_blend = pm.createNode("pairBlend")
    pm.matchTransform(joint_blend, joint)
    pm.parent(joint_blend, pm.listRelatives(joint, p=1)[0])

    pm.makeIdentity(joint_blend, a=1, r=1)
    node_pair_blend.weight.set(0.5)
    node_pair_blend.rotInterpolation.set(1)

    pm.connectAttr("{}.t".format(joint), "{}.inTranslate1".format(node_pair_blend))
    pm.connectAttr("{}.r".format(joint), "{}.inRotate1".format(node_pair_blend))
    pm.connectAttr("{}.t".format(joint), "{}.inTranslate2".format(node_pair_blend))

    pm.connectAttr("{}.outRotate".format(node_pair_blend), "{}.r".format(joint_blend))
...
def create_support_joint(
    ...
    joint_support = pm.createNode("joint", n="{}_support_{}".format(joint, index))

    axis_rot = Misc.get_exist_axis([axis_aim, axis_push])
    axis_push = axis_push
    index = 1

    # create support joint
    Utility.match_parent(joint_support, joint_blend)
    pm.makeIdentity(joint_support, a=1, r=1)

    # add attribute to joint
    pm.addAttr(joint_support, ln="biasX", dv=0, k=True)
    pm.addAttr(joint_support, ln="biasY", dv=0, k=True)
    pm.addAttr(joint_support, ln="biasZ", dv=0, k=True)

    pm.addAttr(joint_support, ln="min".format(index), dv=min, k=True)
    pm.addAttr(joint_support, ln="max".format(index), dv=max, k=True)
    pm.addAttr(joint_support, ln="offset".format(index), dv=offset, k=True)

    pm.addAttr(joint_support, ln="currentRotate".format(index), k=True)
    ...
    # connection
    node_clamp = pm.createNode("clamp")
    node_mdl = pm.createNode("multDoubleLinear")
    node_adl = pm.createNode("addDoubleLinear")
    ...
    pm.connectAttr("{}.outputR".format(node_clamp), "{}.input1".format(node_mdl))
    pm.connectAttr(
        "{}.bias{}".format(joint_support, axis_push.upper()),
        "{}.input2".format(node_mdl),
    )

    pm.connectAttr("{}.output".format(node_mdl), "{}.input1".format(node_adl))
    pm.connectAttr(
    ...
    # connect bias value
    axis_left = Misc.get_exist_axis([axis_push])
    ...
    # set default value
    pm.setAttr("{}.bias{}".format(joint_support, axis_push.upper()), intensity)

    # add joint to mgear sets
    try:
        defSet = pm.PyNode("rig_deformers_grp")
        pm.sets(defSet, add=joint_support)
    except:
        pass
...
def create_corrective_joint_four(
    ...
    for i, index in enumerate(dict_data.keys()):
        ...
        pm.setAttr("{}.r{}".format(joint_support, axis), 90 * i)
        pm.makeIdentity(joint_support, a=1, r=1)

        # add attribute to joint
        pm.addAttr(joint_support, ln="joint_{}_factor".format(index), dv=factor, k=True)
        pm.addAttr(
        ...
        pm.addAttr(joint_support, ln="joint_{}_offset".format(index), dv=offset, k=True)

        pm.addAttr(joint_support, ln="joint_{}_current_rotate".format(index), k=True)
...
def create_corrective_joint(
    ...
    def create_dummy_output():
        ...
        # pm.parent(locator_dummy,grp_corrective_joint)
        grp_frz = create_freeze_group([locator_dummy])[0]

        pm.parentConstraint(locator_dummy, joint_push)
    ...
    def parent_by_matrix(locator, target):
        ...
        node_decomp_matrix = pm.createNode("decomposeMatrix")
        pm.connectAttr(
    ...
    def create_vector_position():
        ...
        pm.parentConstraint(block2, grp_zero)

        # position vector 1
        constraint = pm.parentConstraint(block2, grp_loc1)
        pm.delete(constraint)
        pm.parentConstraint(block1, grp_loc1, mo=1)

        pm.setAttr("{}.t{}".format(locator_1, axis_forward_abs), -1)  # move

        # position vector 2
        pm.parentConstraint(block2, grp_loc2)
        pm.setAttr("{}.t{}".format(locator_2, axis_forward_abs), 1)  # move
        grp_rot = create_freeze_group([locator_2], "grpRot", match_object=locator_zero)[
            0
        ]  # rotate for make 120

        if invert_angle:
            pm.setAttr("{}.r{}".format(grp_rot, axis_side_abs), -60)
        else:
            pm.setAttr("{}.r{}".format(grp_rot, axis_side_abs), 60)

    def create_angle_between():
        # create vector 1
        node_pma_vector1 = pm.createNode(
            "plusMinusAverage", n=Misc.cname(tag_name, "vector1", "pma")
        )
        pm.setAttr("{}.operation".format(node_pma_vector1), 2)
        pm.connectAttr(
            "{}.worldPosition".format(pm.listRelatives(locator_1, c=1, s=1)[0]),
            "{}.input3D[0]".format(node_pma_vector1),
        )
        pm.connectAttr(
            "{}.worldPosition".format(pm.listRelatives(locator_zero, c=1, s=1)[0]),
            "{}.input3D[1]".format(node_pma_vector1),
        )
        attr_vector1 = "{}.output3D".format(node_pma_vector1)

        # create vector 2
        node_pma_vector2 = pm.createNode(
            "plusMinusAverage", n=Misc.cname(tag_name, "vector2", "pma")
        )
        pm.setAttr("{}.operation".format(node_pma_vector2), 2)
        pm.connectAttr(
            "{}.worldPosition".format(pm.listRelatives(locator_2, c=1, s=1)[0]),
        ...
        # invert vector
        # if invert_angle:
        node_md_invert = pm.createNode(
            "multiplyDivide", n=Misc.cname(tag_name, "invertVector", "md")
        )
        pm.connectAttr(attr_vector1, "{}.input1".format(node_md_invert))
        pm.setAttr("{}.input2".format(node_md_invert), -1, -1, -1, typ="double3")
        attr_vector1 = "{}.output".format(node_md_invert)

        # angle between
        node_angle_between = pm.createNode(
            "angleBetween", n=Misc.cname(tag_name, "anglePush", "ab")
        )
        pm.connectAttr(attr_vector1, "{}.vector1".format(node_angle_between))
        pm.connectAttr(attr_vector2, "{}.vector2".format(node_angle_between))

        # set range
        new_min, new_max = new_min_max
        # old_min,old_max = old_min_max

        node_set_range = pm.createNode(
            "setRange", n=Misc.cname(tag_name, "remap", "sr")
        )
        pm.connectAttr(
            "{}.angle".format(node_angle_between), "{}.valueX".format(node_set_range)
        )
        pm.setAttr("{}.minX".format(node_set_range), new_min)
        pm.setAttr("{}.maxX".format(node_set_range), new_max)

        # if invert_angle:
        pm.setAttr("{}.oldMinX".format(node_set_range), 60)
        pm.setAttr("{}.oldMaxX".format(node_set_range), 180)
        # else:
        #     pm.setAttr("{}.oldMinX".format(node_set_range),0)
        #     pm.setAttr("{}.oldMaxX".format(node_set_range),120)
    ...
    def create_hierarchy():
        def create_position_output():
            node_decompose_output = pm.createNode(
                "decomposeMatrix", n=Misc.cname(tag_name, "MatchTransform", "dcm")
            )
            pm.connectAttr(
        ...
        def create_avg_orient_output():
            node_decompose_block1 = pm.createNode(
                "decomposeMatrix",
                n=Misc.cname(tag_name, "{}Rotate".format(block1), "dcm"),
            )
            pm.connectAttr(
            ...
            node_decompose_block2 = pm.createNode(
                "decomposeMatrix",
                n=Misc.cname(tag_name, "{}Rotate".format(block2), "dcm"),
            )
            pm.connectAttr(
                "{}.worldMatrix[0]".format(block2),
            ...
            node_pair_blend = pm.createNode(
                "pairBlend", n=Misc.cname(tag_name, "AvgRotate", "pb")
            )
            pm.setAttr("{}.rotInterpolation".format(node_pair_blend), 1)
            pm.setAttr("{}.weight".format(node_pair_blend), 0.5)
            pm.connectAttr(
            ...
            node_compose_matrix = pm.createNode(
                "composeMatrix", n=Misc.cname(tag_name, "AvgRotate", "cm")
            )
            pm.connectAttr(
            ...
            node_mult_matrix = pm.createNode(
                "multMatrix", n=Misc.cname(tag_name, "offsetMatrix", "mm")
            )
            pm.connectAttr(
            ...
            node_decompose_output = pm.createNode(
                "decomposeMatrix", n=Misc.cname(tag_name, "OutputAvgRotate", "dcm")
            )
            pm.connectAttr(
        ...
        pm.group(em=1, n=grp_corrective_joint)
        pm.setAttr("{}.inheritsTransform".format(grp_corrective_joint), False)

        pm.group(em=1, n=grp_base_orient, p=grp_corrective_joint)
        pm.group(em=1, n=grp_avg_orient, p=grp_base_orient)

        pm.parent(grp_corrective_joint, parent) if parent else None

        pm.spaceLocator(n=locator_1)
        pm.spaceLocator(n=locator_2)
        pm.spaceLocator(n=locator_zero)
        pm.spaceLocator(n=locator_dummy)

        pm.parent(locator_dummy, grp_avg_orient)
        pm.parent(locator_zero, locator_1, locator_2, grp_corrective_joint)

        create_position_output()
        create_avg_orient_output()

    # variables
    grp_corrective_joint = Misc.cname(tag_name, "Corrective")
    grp_base_orient = Misc.cname(tag_name, "BaseOrient")
    grp_avg_orient = Misc.cname(tag_name, "AvgOrient")

    locator_dummy = Misc.cname(tag_name, "dummy")
    locator_1 = Misc.cname(tag_name, "pos1")
    locator_2 = Misc.cname(tag_name, "pos2")
    locator_zero = Misc.cname(tag_name, "posZro")

    axis_forward_abs = Misc.del_neg(axis_forward)
    axis_side_abs = Misc.del_neg(axis_side)
...
def create_ik_reverse(
    ...
    def create_ik_function():
        ...
        def create_hierarchy():
            ...
            pm.parentConstraint(control_parent_ik, grp_local_reverse)
            ...
            for i, joint in enumerate(list_pivot_joint):
                ...
                pm.matchTransform(joint, list_pivot_match[i], pos=1)
                pm.matchTransform(joint, jnt_ball_ik, rot=1)
                pm.makeIdentity(joint, a=1, s=1, r=1)

            pm.parent(list_pivot_joint[0], grp_local_reverse)
            ...
            constraint_toe_orient.append(
                pm.orientConstraint(loc_ball_orient, jnt_ball_ik)
            )
            ...
            constraint = pm.parentConstraint(jnt_ankle_ik, loc_ankle_orient)
            pm.delete(constraint)
            pm.parent(loc_ankle_orient, piv_ball)
            pm.orientConstraint(loc_ankle_orient, jnt_ankle_ik)
            ...
            for object in list_parent_reverse:
                if pm.objExists(object):
                    ...
                    pm.parentConstraint(piv_ankle, object, mo=1)
        ...
        def create_connection():
            ...
            def connect_middle_twist():
                ...
                # Toe tap
                node_adl_toe_tap = pm.createNode(
                ...
                pm.connectAttr(attr_output, "{}.input2".format(node_adl_toe_tap))
        ...
        piv_ankle = pm.joint(n="{}_ankle_piv".format(name_tag))
...
def create_locator_on_curve(amount, list_curve, size=0.5):
    ...
    for curve in list_curve:
        ...
        for i in range(amount):
            locator = pm.spaceLocator(n="loc_" + main_name + "_" + str(i + 1).zfill(2))
...
def ribbon_plane(name, ref, faxis="y", sub=5, drv=3, snap=0):
    ...
    for i in range(len(ref) - 1):
        node_distance = pm.createNode("distanceBetween")
        pm.connectAttr(ref[i] + ".worldMatrix[0]", node_distance + ".inMatrix1")
        pm.connectAttr(ref[i + 1] + ".worldMatrix[0]", node_distance + ".inMatrix2")
        width += pm.getAttr(node_distance + ".distance")
        pm.delete(node_distance)

    # create ribbon plane
    nrb_shape = pm.listRelatives(
        pm.nurbsPlane(
            n="nrb_" + name + "_ribbon",
            ax=(0, 0, 1),
            w=width,
    ...
    for i in range(1, sub + 1):
        flc_shape = pm.createNode("follicle", n=f"flc_{name}_shape_{i}")
        pm.setAttr(flc_shape + ".simulationMethod", 0)
        pm.setAttr(flc_shape + ".parameterV", 0.5)

        value = 1 / (sub - 1)
        pm.setAttr(flc_shape + ".parameterU", value * (i - 1))
        pm.connectAttr(nrb_shape + ".local", flc_shape + ".inputSurface")
        pm.connectAttr(nrb_shape + ".worldMatrix[0]", flc_shape + ".inputWorldMatrix")

        flc_transform = pm.rename(
            pm.listRelatives(flc_shape, p=1, f=1)[0], f"flc_{name}_{i}"
        )
        pm.connectAttr(flc_shape + ".outRotate", flc_transform + ".rotate")
        pm.connectAttr(flc_shape + ".outTranslate", flc_transform + ".translate")
    ...
    for i in range(sub):
        # create sub joint
        jntSub = pm.joint(n=f"jnt_sub_{name}_ribbon_{i + 1}", rad=0.5)
        pm.matchTransform(jntSub, list_flc[i], pos=1, rot=0, scl=0)
    ...
    pm.joint(list_jntSub[0], e=1, oj=oj, sao="xup", ch=1, zso=1)
    pm.joint(list_jntSub[-1], e=1, oj="none", ch=1, zso=1)

    for i in range(sub):
        pm.parent(list_jntSub[i], list_flc[i])

    # move all bind joint to follicles group
    grp_flc = pm.group(em=1, n=f"grp_flc_{name}")
    for i in range(sub):
        pm.parent(list_flc[i], grp_flc)

    # create drv joints
    grp_drv = pm.group(em=1, n=f"grp_jntDrv_{name}")
    offset = pm.xform(list_jntSub[0], q=1, ws=1, t=1)[0]
    list_jntDrv = []

    for i in range(drv):
        drv_value = (i * (width / (drv - 1))) + offset
        pm.select(cl=1)
        jntDrv = pm.joint(n=f"jnt_drv_{name}_ribbon_{i + 1}")
        pm.matchTransform(jntDrv, list_jntSub[0], rot=1)
        pm.setAttr(jntDrv + ".radius", pm.getAttr(jntSub + ".radius") * 3)
        pm.setAttr(jntDrv + ".translateX", drv_value)
        pm.parent(jntDrv, grp_drv)
        list_jntDrv.append(jntDrv)

    # bind skin to nurb
    pm.skinCluster(nrb_shape, list_jntDrv, ih=1, mi=2, n=f"skinCluster_{name}_ribbon")

    if snap == 1 and drv == len(ref):
        for i in range(drv):
            pm.matchTransform(list_jntDrv[i], ref[i])
    ...
    # snap division method
        ...
        for i in range(drv):
            # divide case
            if (i + 1) % 2 == 0:
                rot = pm.xform(ref[count - 1], q=1, ws=1, ro=1)
                pos1 = pm.xform(ref[count - 1], q=1, ws=1, t=1)
                pos2 = pm.xform(ref[count], q=1, ws=1, t=1)

                avg_pos = [0, 0, 0]
                for axis in range(3):
                    avg_pos[axis] = (pos1[axis] + pos2[axis]) / 2

                pm.xform(list_jntDrv[i], ws=1, t=avg_pos, ro=rot)

            # point case
            else:
                pm.matchTransform(list_jntDrv[i], ref[count])
                count += 1
