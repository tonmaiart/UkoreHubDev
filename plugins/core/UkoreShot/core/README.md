...
- `video_path_store.py` — reads the active repo's Custom Paths off
  ...
  **Custom Paths** (Repository Setting > Custom Paths > Create Input Path,
  ...
  convention. `plugins/core/UkorePlayblast/`'s Maya-side `function.py`
...
data shape / `plugins/core/UkorePlayblast/`'s output (both read-only,
