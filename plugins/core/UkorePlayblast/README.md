...
`plugins/core/maya_launcher/README.md`. Replaces UkoreMaya's old
...
whichever folder `plugins/core/UkoreShot/` has configured for the active
...
- **index**/**version** — see "Versioning" below. Both, along with
  ...
  `_FILENAME_PATTERN` and `plugins/core/UkoreShot/video_naming.py` (the
...
`plugins/core/UkoreShot/video_naming.py`'s parser treats anything that
...
- `maya-scripts/UkorePlayblast/function.py` — `publish_playblast()`,
  what `plugins/core/MayaToolkit`'s UkoreMaya menu now calls (see that
  ...
  `plugins/core/UkoreShot/video_path_store.py`'s `resolve_video_root`
...
`plugins/core/MayaToolkit/maya-scripts/UkoreMaya/core/menu_utils.py` has
...
`options_dialog.show()`). `plugins/core/MayaToolkit/maya-plug-ins/ukoreMaya.py`
