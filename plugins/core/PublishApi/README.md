...
`PYTHONPATH` entry to `plugins/core/maya_launcher/`'s shared
...
**Never gated by `plugins/core/maya_launcher/`'s per-repo `RepoToolsStore`
...
- `plugin.py` — `register(api)`: contributes `maya-scripts/` **and**
  ...
  `plugins/core/UkoreBrowser/plugin.py` contributes `api.app_root` too —
...
- Each ticket has a list of attached **validation scripts** —
  ...
  `plugins/core/MayaToolkit/maya-scripts/tmlib/core/QuickData.py`'s
...
still present in `plugins/core/MayaToolkit/` — unrelated tools may still
