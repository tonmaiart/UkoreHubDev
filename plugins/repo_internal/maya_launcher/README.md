...
`plugins/repo_internal/ModelPublisher/`, `plugins/repo_internal/RigPublisher/`,
`plugins/repo_internal/AnimationPublisher/` — each with its own dedicated UI
instead of one shared "pick a Type, then a Ticket" window. All three are
built on `plugins/repo_internal/PublishApi/`, a new non-UI Maya-side library
...
| `plugins/repo_internal/ModelPublisher` | `PYTHONPATH` |
| `plugins/repo_internal/RigPublisher` | `PYTHONPATH` |
| `plugins/repo_internal/AnimationPublisher` | `PYTHONPATH` |
...
shared=True)` → `data/plugins/repo_internal/maya_launcher.json`, key shape
...
**`plugins/repo_internal/PublishApi` is never gated by this at all**, on top of
...
script that imports `plugins.repo_internal.maya_launcher.repo_tools_store`
