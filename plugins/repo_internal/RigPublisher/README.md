# plugins/repo_internal/RigPublisher/
...
library — see `plugins/repo_internal/PublishApi/README.md` and
...
- `plugin.py` — `register(api)`: contributes `maya-scripts/` to the shared
  ...
  other Maya tool plugin here uses). Relies on `plugins/repo_internal/MayaToolkit`
...
- `maya-scripts/RigPublisher/interface.py` — `MainWindow`
  ...
  `plugins/repo_internal/ModelPublisher/`'s own `interface.py` — ticket list +
...
Same as `plugins/repo_internal/ModelPublisher/README.md`'s "What changed" section
...
`plugins/repo_internal/PublishApi/README.md`'s "Tickets" section for the full
