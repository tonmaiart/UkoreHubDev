...
scene-path convention — see plugins/repo_internal/PublishApi/README.md. As of
...
def publish(ticket: dict):
    """Exports the current scene as a raw Maya Ascii copy into this
    ticket's own resolved publish root, versioned automatically (mirrors
    the original UkorePublisher's Rig branch: "Rig, Proxy, Hi > Maya
    Ascii", just with user-managed tickets instead of a hardcoded list).
    Raises RuntimeError — meant to be shown directly to the artist — if
    the scene hasn't been saved, one of the ticket's validation scripts
    blocks the publish, or the ticket's own Publish Path can't be
    resolved. Returns (version_dir, version_number)."""
