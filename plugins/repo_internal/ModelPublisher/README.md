# plugins/repo_internal/ModelPublisher/
...
- `maya-scripts/ModelPublisher/ui.ui` — Qt Designer layout, loaded by
  ...
  convention `plugins/repo_internal/UkoreBrowser/`'s own `ui.ui` uses).
...
- The free-text "Custom Path" field artists used to type in Maya (added
  ...
  (`plugins/core/project_editor`'s "Custom Paths" tab), not something an
